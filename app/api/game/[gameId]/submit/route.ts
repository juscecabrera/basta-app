...
export async function POST(req: NextRequest) {
  ...
  const { playerId, answers } = await req.json();

  if (typeof gameId !== 'string' || !playerId || !answers) {
  ...
  if (games[gameId].stopped) {
    return NextResponse.json({ error: 'Game is stopped' }, { status: 400 });
  }
